    ...
    async *stream(request) {
      ...
      const response = await fetch(`${config.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          tools: request.tools.length === 0 ? undefined : request.tools.map(toProviderTool),
        }),
...
function resolveConfig(options: LlmPluginOptions) {
  const apiKey = options.apiKey ?? process.env.ATOM_LLM_API_KEY ?? "";
  const baseUrl = (options.baseUrl ?? process.env.ATOM_LLM_BASE_URL ?? "").replace(/\/$/, "");
  const model = options.model ?? process.env.ATOM_LLM_MODEL ?? "";
  if (!apiKey || !baseUrl || !model) {
    throw new Error("llm 适配器未配置 apiKey、baseUrl 或 model");
  }
...
type ParsedFrame =
  | { readonly kind: "chunk"; readonly chunk: LlmChunk }
